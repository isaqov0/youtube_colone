...
const VideoDetail = () => {
  ...
  const [relativedVideo, setRelativedVideo] = useState([]);
  const { id } = useParams();
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const relativeData = await ApiService.fetching(
          `search?part=snippet&relatedToVideoId=${id}&type=video`
        );

        setRelativedVideo(relativeData.items);
      } catch (error) {
        console.log(error);
      }
    };
            ...
            {tags &&
              tags.map((item, idx) => (
                <Chip
                  label={`#${item}`}
                  key={idx}
                  sx={{ marginTop: "10px", curser: "pointer", ml: "10px" }}
                  deleteIcon={<Tag />}
                  onDelete={() => {}}
                  variant="outlined"
                />
